...
def parse_args(args):
    """ Parse arguments from command line input     解析命令行输入中的参数
    """
...
def main(args=None):
    ...
    # Check if a GPU ID was set 			检查是否设置了GPU ID
    if args.gpu:
        os.environ['CUDA_VISIBLE_DEVICES'] = args.gpu
    set_session(get_session())
    summary_writer = tf.summary.FileWriter(args.type + "/tensorboard_" + args.env)

    # Environment Initialization			环境初始化
    if (args.is_atari):
        # Atari Environment Wrapper			Atari环境包装器
        env = AtariEnvironment(args)
        state_dim = env.get_state_size()
        action_dim = env.get_action_size()
    elif (args.type == "DDPG"):
        # Continuous Environments Wrapper	连续环境包装
        ...
        # Standard Environments 			标准环境
        env = Environment(gym.make(args.env), args.consecutive_frames)
        env.reset()
        state_dim = env.get_state_size()
        action_dim = gym.make(args.env).action_space.n

    # Pick algorithm to train 				选择算法进行训练
    if (args.type == "DDQN"):
